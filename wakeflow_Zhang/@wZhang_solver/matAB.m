...
switch obj.method(1)
    ...
    %% Rayleigh
        ...
        A(N:2*N-2, N+2:end-1) = A_ge(N+3:end-1,:);
        B_ge = D(:,:,3) - k^2*D(:,:,1);
        B(1:N-1, 1:N+1) = B_ge(2:N,:);
        B(N:2*N-2, N+2:end-1) = B_ge(N+3:end-1,:);
        % KFSBC
        A(end-4, 1:N+1) = k*D(1,:,1);
        A(end-4, end) = k*U(1,1);
        B(end-4, end) = 1;
        % DFSBC
        A(end-3, 1:N+1) = k*U(1,1).*D(1,:,2) - k*U(1,2).*D(1,:,1);
        A(end-3, end) = k/obj.Fr2;
        B(end-3, 1:N+1) = D(1,:,2);
        % z = -h
        A(end-2, N+2:end-1) = D(end,:,1);
        % Matching condition
        A(end-1:end, 1:end-1) = permute([D(N+1,:,1:2) -D(N+2,:,1:2)],[3 2 1]);
    case 'd4'
      %% D4
        % Governing equation
