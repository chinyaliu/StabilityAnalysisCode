...
switch lower(method)
    ...
    case 'uw'
        ...
        Abc1 = [zeros(1,N+1) 1i*BC{1}(1,:) k*BC{3}(1)];
        Abc2 = [BC{1}(2,:) 1i*k*BC{1}(1,:) BC{3}(3)];
    ...
    case 'd4'
      %% D4
        ...
        Abc1 = [k*BC{1}(1,:) k*BC{3}(1)];
        Abc2 = [BC{1}(3,:) + k^2*BC{1}(1,:) BC{3}(3)];
    ...
    case 'ray'
      %% Reyleigh
        % Matrix A
        A1 = k*U(:,1).*D(:,:,3) - (U(:,1)*k^3 + U(:,3)*k).*D(:,:,1);
        Abc1 = [k*BC{1}(1,:) k*BC{3}(1)];
        Abc3 = [k*BC{3}(1).*BC{1}(2,:) - k*BC{3}(2).*BC{1}(1,:) k/Fr2];
        A = [[A1 zeros(N-1,1)]; Abc1; Abc3; [BC{2}(1,:) 0];];
        % Matrix B
        B = zeros(N+2, N+2);
        B(1:N-1,1:N+1) = D(:,:,3) - k^2*D(:,:,1);
        B(N+1,1:N+1) = BC{1}(2,:);
        B(N,end) = 1;
    otherwise
